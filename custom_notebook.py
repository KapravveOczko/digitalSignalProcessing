...
class CustomNotebook(ttk.Notebook):
    ...
    def create_operatin_on_signals_widgets(self):
        ...
        operation_label = ttk.Label(operation_frame, text="Wybierz Operacje")
        operation_label.grid(column=0, row=row_number)
    ...
    def create_generate_signals_widgets(self):
        ...
        row_number += 1
        hist_bins_label = ttk.Label(signal_params_frame, text="Przedział histogramu:")
        hist_bins_label.grid(row=row_number, column=0, sticky="w")
    ...
    def update_tab_list(self):
        ...
        if self.tab_names:
            self.first_tab_menu.set(self.tab_names[0])
            self.second_tab_menu.set(self.tab_names[0])
            self.first_tab.set(self.tab_names[0])
            self.second_tab.set(self.tab_names[0])
